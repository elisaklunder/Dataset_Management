    ...
    def train_test_split(self, train_size, test_size, shuffle):
        ...
        if shuffle:
            if bool(self._targets):
                zipped_data = list(
                    zip(self.data, self.targets)
                )  # using the getter here to not fuck with the original data
                shuffled_data = random.sample(zipped_data, data_len)
                data, targets = zip(
                    *shuffled_data
                )  # using local variables makes sure that the original dataset won't be unintentionally shuffled when doing trian test split
            else:
                data = random.sample(
                    self.data, data_len
